...
export default function Messages() {
  ...
  // Refresh the conversation list (previews, unread counts, ordering) as
  // new messages arrive anywhere, even while a different thread is open.
  useEffect(() => {
  ...
  async function startConversation(patientId) {
    setError('');
    try {
      const { conversation } = await api.chat.startConversation({ patient_id: patientId });
  ...
  const contactedPatientIds = new Set(conversations.map((c) => c.patient_id));
  ...
  return (
    <div>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          {contacts.length === 0 ? (
            <div style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>
              No patients yet — you can message a patient once you have an appointment or admission with them.
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {contacts.map((c) => (
                <button
                  ...
                  onClick={() => startConversation(c.patient_id)}
                >
                  {c.full_name} <span className="mono" style={{ color: 'var(--muted)' }}>{c.patient_code}</span>
                  {contactedPatientIds.has(c.patient_id) ? ' — open thread' : ''}
                </button>
              ))}
            </div>
          )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 16 }}>
            ...
            conversations.map((c) => (
              <div
                ...
                style={{
                  ...
                  background: c.id === selectedId ? 'var(--sky-100)' : 'transparent',
                }}
              >
        ...
        {selected ? (
          <ChatWindow conversation={selected} currentUserId={user.id} />
        ) : (
